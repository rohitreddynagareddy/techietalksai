...
async def _research_impl(name: str) -> list[types.TextContent]:
    """Internal function to handle the booking logic."""
    logger.debug(f"Research on name: {name}")
    ...
    server_agent = Agent(
        # 'anthropic:claude-3-5-haiku-latest', system_prompt='always reply in rhyme'
        model=model, system_prompt='Do a deep research to generate a detailed report, use tables to display data'
    )
    """Poem generator"""
    r = await server_agent.run(f'Do a research on {name} and generate a report')
    # return r.data


    if not name or not isinstance(name, str):
         logger.error("Invalid 'name' provided for research.")
         # Return error information as text content
         return [types.TextContent(type="text", text="Error: Invalid or missing 'name' for research.")]
