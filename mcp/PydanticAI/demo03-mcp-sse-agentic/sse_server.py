...
async def _poem_impl(name: str) -> list[types.TextContent]:
    ...
    logger.debug(f"Creating a poem on name: {name}")
    if not name or not isinstance(name, str):
         logger.error("Invalid 'name' provided for the poem.")
         # Return error information as text content
         return [types.TextContent(type="text", text="Error: Invalid or missing 'name' for poem.")]

    agent_run = await agent.run(f"Create a funny short poem for kids with a moral on: {name}")
    logger.info(f"Generated poem on '{name}': '{agent_run.data}'")
    return [types.TextContent(type="text", text=agent_run.data)]
...
def main(port: int, transport: str) -> int:
    ...
    async def handle_tool_call(
        ...
        if name == "poem":
            if "name" not in arguments:
                ...
                return [types.TextContent(type="text", text="Error: Missing required argument 'name' for poem tool.")]
            ...
            # Return error message in the expected format
            return [types.TextContent(type="text", text=f"Error: Unknown tool '{name}'. Available tools: poem.")]
    ...
    async def list_available_tools() -> list[types.Tool]:
        ...
        return [
            ...
            types.Tool(
                name="poem",
                description="Creates a poem for the specified topic name",
                inputSchema={
                    "type": "object",
                    "required": ["name"],
                    "properties": {
                        "name": {
                            "type": "string",
                            "description": "The name of the topic for the poem to be made.",
                        }
                    },
                },
