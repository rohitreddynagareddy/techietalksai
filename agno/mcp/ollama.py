...
async def create_filesystem_agent(session):
    ...
    return Agent(
        # gemma3:1b does not support tools
        # model=Ollama(id="gemma3:1b", host="http://host.docker.internal:11434"),
        # WORKS
        model=Ollama(id="llama3.2:latest", host="http://host.docker.internal:11434"),
        # model=Ollama(id="phi4-mini:latest", host="http://host.docker.internal:11434"),
        # NO TOOL CALLS
        # model=Ollama(id="deepseek-r1:latest", host="http://host.docker.internal:11434"),
        tools=[mcp_tools],
        instructions=dedent("""\
