...
with col1:
    image_url = st.text_input("Image URL:", placeholder="Paste image URL here")
with col2:
    prompt = st.text_input("Analysis prompt:", placeholder="e.g., 'Latest news about this location'")

if image_url and prompt:
    st.session_state.analysis_count += 1
    
    # Display the image at the top
    st.image(image_url, caption=f"Analyzing image: {image_url}", use_container_width=True)
    st.markdown("---")
    ...
    with st.spinner("🔍 Analyzing image and researching context..."):
        ...
        if stream:
            ...
            response = agent.run(prompt, images=[Image(url=image_url)], stream=False)
