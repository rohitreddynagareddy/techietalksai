...
def generate_summary(text):
    ...
    a = asyncio.run(run_agent_query(agent,  f"Create concise bullet point summaries from this text:\n{text} and save the results to /app/app/results/summary.txt"))
    b = asyncio.run(run_agent_query(agent,  f"Create concise bullet point summaries from this text:\n{text}"))
    return a + b 
...
def generate_quiz(text):
    agent = Agent(
        ...
        mcp_servers=[server],
        # system_prompt=system_prompt,
        instructions="""Generate 5 multiple choice questions based on the text. 
    ...
    return asyncio.run(run_agent_query(agent,  f"{text}"))
