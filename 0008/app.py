...
@agent.tool
async def retriever(
    # ctx: RunContext[Deps], operation: str, num1: float, num2: float
    ...
    conversation = []
    conversation.append({"role": "user", "content": f"{query}"})  # VERIFY
    conversation.append({"role": "assistant", "content": observation_text}) # VERIFY
